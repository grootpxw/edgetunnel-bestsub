...
func GenerateToLocalProfile(cfg config.Config, results []probe.Result) (GenerateResult, error) {
	...
	if cfg.Clash.AutoRegister {
		if err := registerClashProfile(cfg); err != nil {
			return GenerateResult{}, err
		}
		registered = true
	}
	return GenerateResult{Path: path, Nodes: len(success), Registered: registered}, nil
}
...
func wsPath(cfg config.Config, index int) string {
	...
	if cfg.Clash.EarlyData > 0 {
		...
		query += fmt.Sprintf("ed=%d", cfg.Clash.EarlyData)
	}
	if query != "" {
...
type profileEntry struct {
	...
	Option  map[string]any `yaml:"option,omitempty"`
	Extra   map[string]any `yaml:",inline"`
}

func registerClashProfile(cfg config.Config) error {
	dir := strings.TrimSpace(cfg.Clash.LocalProfileDir)
	if dir == "" {
		return fmt.Errorf("未配置 clash.local_profile_dir，不能注册 Clash profile")
	}
	path := filepath.Join(dir, "..", "profiles.yaml")
	if _, err := os.Stat(path); err != nil {
		return fmt.Errorf("未找到 Clash Verge profiles.yaml：%s", path)
	}
	...
	uid := strings.TrimSpace(cfg.Clash.ProfileUID)
	if uid == "" {
		uid = "bestsub-local"
	}
	name := strings.TrimSpace(cfg.Clash.ProfileName)
	if name == "" {
		name = "BestSub 本地优选"
	}
	filename := strings.TrimSpace(cfg.Clash.Filename)
	...
	entry := profileEntry{
		...
		Name:    name,
		File:    filename,
		Desc:    "Generated by EdgeTunnel BestSub",
