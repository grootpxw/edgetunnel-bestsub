...
let _cache = { userID: null, hosts: null, proxyIPs: null, proxyEnabled: null, config: null, configTime: 0, trojanHash: null, socks5List: null };
const _CONFIG_TTL = 60000;
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (!_cache.userID) {
            ...
            _cache.userID = (envUUID && uuidRegex.test(envUUID)) ? envUUID.toLowerCase() : [userIDMD5.slice(0, 8), userIDMD5.slice(8, 12), '4' + userIDMD5.slice(13, 16), '8' + userIDMD5.slice(17, 20), userIDMD5.slice(20)].join('-');
        }
        const userID = _cache.userID;
        if (!_cache.hosts) {
            _cache.hosts = env.HOST ? (await 整理成数组(env.HOST)).map(h => h.toLowerCase().replace(/^https?:\/\//, '').split('/')[0].split(':')[0]) : null;
        }
        const hosts = _cache.hosts || [url.hostname];
        const host = hosts[0];
        const 访问路径 = url.pathname.slice(1).toLowerCase();
        调试日志打印 = ['1', 'true'].includes(env.DEBUG) || 调试日志打印;
        let KV_PROXYIP = null;
        if (env.KV && typeof env.KV.get === 'function') KV_PROXYIP = await env.KV.get('PROXYIP.txt');
        const PROXYIP_SOURCE = (KV_PROXYIP && KV_PROXYIP.trim()) ? KV_PROXYIP : env.PROXYIP;
        if (PROXYIP_SOURCE) {
            if (!_cache.proxyIPs || _cache.proxySource !== PROXYIP_SOURCE) {
                _cache.proxyIPs = await 整理成数组(PROXYIP_SOURCE);
                _cache.proxyEnabled = true;
                _cache.proxySource = PROXYIP_SOURCE;
            }
            反代IP = _cache.proxyIPs[Math.floor(Math.random() * _cache.proxyIPs.length)];
            启用反代兜底 = false;
        } else 反代IP = (request.cf.colo + '.proxyip.cmliusss.net').toLowerCase();
        const 访问IP = request.headers.get('CF-Connecting-IP') || request.headers.get('True-Client-IP') || request.headers.get('X-Real-IP') || request.headers.get('X-Forwarded-For') || request.headers.get('Fly-Client-IP') || request.headers.get('X-Appengine-Remote-Addr') || request.headers.get('X-Cluster-Client-IP') || '未知IP';
        if (env.GO2SOCKS5) {
            if (!_cache.socks5List) _cache.socks5List = await 整理成数组(env.GO2SOCKS5);
            SOCKS5白名单 = _cache.socks5List;
        }
        if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
            ...
            if (env.KV && typeof env.KV.get === 'function') {
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    } else if (访问路径 === 'admin/proxyip-test') {
                        const proxyip = url.searchParams.get('proxyip');
                        const target = url.searchParams.get('url') || 'https://cloudflare.com/cdn-cgi/trace';
                        if (!proxyip) return new Response(JSON.stringify({ success: false, error: 'missing proxyip' }, null, 2), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        try {
                            const result = await testProxyIPExit(proxyip, target);
                            return new Response(JSON.stringify(result, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        } catch (err) {
                            return new Response(JSON.stringify({ success: false, proxyip, url: target, error: err.message }, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        }
                    ...
                    config_JSON = await 读取config_JSON缓存(env, host, userID, UA);

                    if (访问路径 === 'admin/init') {
                        try {
                            config_JSON = await 读取config_JSON(env, host, userID, UA, true);
                            _cache.config = config_JSON;
                            _cache.configTime = Date.now();
                        ...
                        if (访问路径 === 'admin/config.json') { // 保存config.json配置
                            try {
                                ...
                                _cache.config = null; _cache.configTime = 0;
                        ...
                        } else if (区分大小写访问路径 === 'admin/PROXYIP.txt') {
                            try {
                                const proxyIPs = (await request.text()).trim();
                                await env.KV.put('PROXYIP.txt', proxyIPs);
                                _cache.proxyIPs = null;
                                _cache.proxyEnabled = null;
                                _cache.proxySource = null;
                                ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Save_Proxy_IPs', config_JSON));
                                return new Response(JSON.stringify({ success: true, message: 'PROXYIP saved' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            } catch (error) {
                                console.error('保存PROXYIP失败:', error);
                                return new Response(JSON.stringify({ error: '保存PROXYIP失败: ' + error.message }), { status: 500, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            }
                        ...
                        const proxyIPs = await env.KV.get('PROXYIP.txt') || '';
                        return new Response(proxyIPs, { status: 200, headers: { 'Content-Type': 'text/plain;charset=utf-8', 'Cache-Control': 'no-store' } });
...
async function 读取XHTTP首包(reader, token) {
    ...
    const 尝试解析木马首包 = (data) => {
        if (!_cache.trojanHash) _cache.trojanHash = new TextEncoder().encode(sha224(token));
        const 密码哈希字节 = _cache.trojanHash;
        const length = data.byteLength;
...
async function testProxyIPExit(proxyip, targetURL) {
    const startedAt = Date.now();
    const parsedTarget = new URL(targetURL || 'https://cloudflare.com/cdn-cgi/trace');
    if (parsedTarget.protocol !== 'https:') throw new Error('only https target url is supported');

    const proxy = parseProxyIPHostPort(proxyip);
    const targetHost = parsedTarget.hostname;
    const targetPort = parsedTarget.port ? Number(parsedTarget.port) : 443;
    ...
    try {
        ...
        await Promise.race([
            socket.opened,
            new Promise((_, reject) => setTimeout(() => reject(new Error('proxyip connect timeout')), 3000))
        ]);

        tlsSocket = new TlsClient(socket, { serverName: targetHost, insecure: false });
        await Promise.race([
            tlsSocket.handshake(),
            new Promise((_, reject) => setTimeout(() => reject(new Error('tls handshake timeout')), 5000))
        ]);

        await tlsSocket.write(encoder.encode(`GET ${path} HTTP/1.1\r\nHost: ${targetHost}\r\nUser-Agent: EdgeTunnel-BestSub-ProxyIP-Test/1.0\r\nAccept: application/json,text/plain,*/*\r\nConnection: close\r\n\r\n`));

        let responseBuffer = new Uint8Array(0);
        const maxBytes = 128 * 1024;
        const deadline = Date.now() + 8000;
        while (responseBuffer.length < maxBytes && Date.now() < deadline) {
            ...
            responseBuffer = concatProxyIPTestBytes(responseBuffer, chunk);
            const text = decoder.decode(responseBuffer);
            if (text.includes('\r\n0\r\n\r\n') || text.includes('\r\n\r\n')) {
                const headerEnd = text.indexOf('\r\n\r\n');
                if (headerEnd !== -1 && !/\r\nTransfer-Encoding:\s*chunked/i.test(text)) {
                    const lengthMatch = text.slice(0, headerEnd).match(/\r\nContent-Length:\s*(\d+)/i);
                    if (!lengthMatch || responseBuffer.length >= headerEnd + 4 + Number(lengthMatch[1])) break;
                }
            }
        }

        const raw = decoder.decode(responseBuffer);
        ...
        if (!status || status < 200 || status >= 300) throw new Error(`target returned HTTP ${status || 'unknown'}`);

        const bodyText = decodeProxyIPTestBody(headerText, raw.slice(headerEnd + 4));
        const parsedBody = parseProxyIPTestBody(bodyText);
        return {
            success: true,
            proxyip: `${proxy.hostname}:${proxy.port}`,
            url: parsedTarget.href,
            ip: parsedBody.ip,
            country: parsedBody.country,
            colo: parsedBody.colo,
            status,
...
function parseProxyIPHostPort(value) {
    value = String(value || '').trim().split('#')[0];
    if (!value) throw new Error('empty proxyip');
    if (/^\[[^\]]+\]:\d+$/.test(value)) {
        const end = value.indexOf(']');
        return { hostname: value.slice(1, end), port: Number(value.slice(end + 2)) };
    }
    const lastColon = value.lastIndexOf(':');
    if (lastColon <= 0) return { hostname: value, port: 443 };
    const hostname = value.slice(0, lastColon);
    const port = Number(value.slice(lastColon + 1));
    if (!Number.isInteger(port) || port <= 0 || port > 65535) throw new Error('invalid proxyip port');
    return { hostname, port };
}

function concatProxyIPTestBytes(a, b) {
...
function decodeProxyIPTestBody(headers, body) {
    if (!/\r\nTransfer-Encoding:\s*chunked/i.test(headers)) return body;
    return body.replace(/(?:^|\r\n)[0-9a-fA-F]+\r\n/g, '').replace(/\r\n0\r\n\r\n?$/, '');
}

function parseProxyIPTestBody(bodyText) {
    let raw = bodyText;
    try {
        raw = JSON.parse(bodyText);
        const ip = raw.clientIp || raw.ip || raw.query || raw.address || null;
        const country = raw.country || raw.countryCode || raw.loc || raw.country_code || null;
        const colo = raw.colo || raw.coloCode || null;
        return { ip, country, colo, raw };
    } catch (e) {
        const ip = bodyText.match(/(?:^|\n)(?:ip|clientIp|query)\s*[=:]\s*"?([0-9a-fA-F:.]+)/)?.[1] || null;
        const country = bodyText.match(/(?:^|\n)(?:country|loc|countryCode)\s*[=:]\s*"?([A-Z]{2})/)?.[1] || null;
        const colo = bodyText.match(/(?:^|\n)(?:colo)\s*[=:]\s*"?([A-Z0-9]{3,4})/)?.[1] || null;
        return { ip, country, colo, raw };
    }
}
...
async function 读取config_JSON缓存(env, hostname, userID, UA) {
    const now = Date.now();
    if (_cache.config && (now - _cache.configTime) < _CONFIG_TTL) return _cache.config;
    const cfg = await 读取config_JSON(env, hostname, userID, UA);
    _cache.config = cfg;
    _cache.configTime = now;
    return cfg;
}
